...
## 14. Remaining Paper 1 figures (C2/C3/Figure 5, `docs/science/paper1-detailed-work-breakdown.md`)
...
- ~~Post-SN secondary Roche-lobe overflow not modelled (found
  ...
  `binaries/wind_capture.py`) now exist and are independently tested,
  but are NOT yet wired into `evolve()`'s HMXB-activation pipeline --
  that wiring decision (how `Mdot_acc` becomes `L_X`, and how it
  interacts with the existing `fsur`/`use_post_sn_rlof` channels)
  remains open, the explicit next step.
