...
| **Post-SN secondary Roche-lobe overflow (`docs/science/paper1-followup-prompt.md`, opt-in `config.use_post_sn_rlof`, default `False`)** | Not from Power et al. -- the physically dominant real HMXB-formation channel this session's earlier work found missing entirely: the secondary's own later RLOF onto the by-then-compact primary (Case B/C mass transfer), as opposed to `fsur`'s phenomenological wind-fed-HMXB approximation | `binaries/population.py::evolve`, new "Phase 1.5" (`nturn==1`, checked live every timestep via the same Eggleton Roche-lobe fit already used for Phase 0 -- `roche_lobe_radius`, `main_sequence.ms_radius`/`hg_radius`/`phase` -- rather than precomputed, since `nturn==1` is already the correct gate and no root-finder ordering problem exists the way it did for Phase 0) | **unit-tested, sensitivity-verified, confirmed genuinely additive**: `tests/test_post_sn_rlof.py` covers the disabled-by-default inert state, that a tight enough orbit triggers within the secondary's own MS+HG lifetime, that a wide enough one never does, that the channel is correctly gated on `nturn==1` (not before the primary's SN, not after the secondary's own), `is_survived` (a disrupted binary has no bound orbit for RLOF), the `mcomp` floor (matching the existing `fsur`-activation gate's own), that it never overwrites an already-active system (from either this channel or the stochastic `fsur` draw), and graceful Z=0 (imetal=1) handling. I verified sensitivity directly, twice: loosened the `nturn==1` gate to `nturn<=2`, confirmed the post-secondary-SN test fails exactly as expected, reverted; removed the `mcomp` floor check, confirmed that test fails too, reverted. Confirmed a real, non-degenerate, additive effect on a realistic population (`ntot=100000`, `mcut=8`, `fsur=0.5`): final `lumx_tot` at t=100 Myr goes from `3.74e40` (channel off) to `4.83e40` (channel on) -- roughly +29%, not a no-op. Negligible runtime overhead confirmed (~11.6s for the full run, same order as without). **Deliberately minimal scope, not a full treatment**: a single RLOF-only trigger, no secondary mass-loss/envelope-stripping consequence, no compact-primary mass growth (Hovis-Afflerbach et al. 2025's stripped-donor properties remain a separate, documented extension point, not attempted here) -- on trigger, activation becomes certain (an `L_X` draw via the existing `xray_calc`), not a stochastic `fsur`-style draw. **Wind accretion is a genuinely separate, NOT-implemented channel**: real wind-fed HMXBs (the classical Vela X-1-type systems) don't require Roche-lobe overflow at all -- a massive OB secondary's own line-driven wind, captured by the compact primary via Bondi-Hoyle-Lyttleton accretion, operates continuously through the secondary's MS lifetime independent of any Roche-lobe condition. Explicitly scoped out here (by user decision, 2026-08-25) pending its own literature-verification pass (a wind mass-loss-rate prescription, e.g. Vink et al. 2001, plus the Bondi-Hoyle-Lyttleton accretion-rate formula and an accretion efficiency) -- not implemented from memory, following this session's established discipline. |
...
| **Population-level literature cross-check (B2, `docs/science/paper1-detailed-work-breakdown.md`)** | Xu et al. (2025, A&A 704, A218, arXiv:2503.23876)'s SMC statistics: `8%` post-mass-transfer, `7%` merger, for a sample with `M1=5-100` Msun, `q=0.3-0.95`, `P=1-3162` d -- a sanity check, not an exact-match requirement, per the task's own wording | `scripts/xu2025_smc_crosscheck.py` (generates a comparable-setup population, post-filters to Xu et al.'s exact `q` window since Realta has no generation-time `q_min`/`q_max` knob, resolves `COMMON_ENVELOPE` outcomes via `apply_common_envelope` for a genuine survive/merge split) | **done, a real (not tuned-away) discrepancy found and traced to an already-known cause**: Realta gives `post-mass-transfer=1.6%`, `merger=29.5%` against Xu et al.'s `8%`/`7%` -- same order of magnitude, but systematically merger-heavy. Traced directly, not assumed: confirmed `100%` of merger cases in the cross-check sample have the *heavier* star as the RLOF donor (`rlof_donor_is_star1` all `True`), exactly the already-documented `find_rlof_onset` emergent property (Eggleton's `R_L1/a` increasing monotonically with the donor's own mass ratio, so the automatically-selected donor is almost always the more massive star). For Xu et al.'s own `q` window, that means the donor's `q1=M_donor/M_companion` is *always* `>1` (`1.05-3.33` across `q=0.3-0.95`), comfortably above `Q_CRIT_MS=0.695`/the HG `q_crit` formula essentially every time -- so this classifier's donor-selection logic systematically under-produces stable mass transfer relative to observations in this regime. Deliberately NOT tuned away by adjusting `Q_CRIT_MS`/`ALPHA_CE`/`LAMBDA_CE` to fit this one comparison point (that would be curve-fitting against a single external number, not a physics improvement) -- recorded as a genuine, understood limitation instead. `~22%` of the selected sample falls to `PHASE_NOT_MODELLED` (GB/AGB donors beyond the classifier's MS/HG scope), a separate, already-documented limitation. Metallicity caveat: `imetal=2` (`Z=0.008`) is the closer of Realta's two usable presets to SMC's true `Z~0.002-0.004`, not an exact match. See `docs/science/rlof-ce-classifier-proposal.md`'s "Population-level check" note for the full writeup. |
...
| Wiring into `evolve()`'s Phase 0 | n/a | `binaries/population.py::evolve` | **unit-tested, sensitivity-verified, one bug found and fixed once HG search was added**: `tests/test_rlof_wiring.py` confirms `evolve()` actually applies the transfer (mass conservation, correct direction, orbit widening) and does not reprocess an already-applied event on a later timestep. The donor's lifetime clock (`turnoff_time` or `t2_lifetime`, whichever is its) is still a full reset from `tnow` at its new mass -- no verified response prescription exists for a mass-*losing* donor, so this remains a named simplification. The companion's clock is now rejuvenated instead of reset -- see the new row below (B3). I verified sensitivity directly: removed the orbital-separation update, confirmed the widening assertion fails, reverted. Originally computed the donor's radius via `ms_radius()` unconditionally; once `find_rlof_onset` could also find a stable-MT crossing during HG, this broke (see Section 10's `find_rlof_onset` row) -- fixed by checking `main_sequence.phase()` at `rlof_time` and dispatching to `ms_radius()`/`hg_radius()` accordingly. |
| **MS-mass-gainer rejuvenation (B3, `docs/science/paper1-detailed-work-breakdown.md`)** | Tout, Aarseth, Pols & Eggleton (1997, MNRAS 291, 732), Sec. 5.1 "Rejuvenation", eq. (41) -- verified directly against the paper (not from memory) before implementing, after first checking HTP02 (2002) Sec. 2.6.6.1 and Hurley, Pols & Tout (2000) Sec. 7.1, both of which cite this same paper's formula for the case relevant here rather than reproducing it in full | `binaries/interaction.py::rejuvenate_ms_gainer`, wired into `evolve()`'s `STABLE_MASS_TRANSFER` branch (replaces the companion's own full-reset; the donor's clock is unaffected, still a full reset -- see the row above) | **unit-tested, sensitivity-verified**: `tests/test_ms_gainer_rejuvenation.py` confirms the radiative-core case (`0.3<M<1.3` Msun) reduces to the simple fractional-age-preservation formula (independently verified against HTP02/Paper I Sec. 7.1's own `t'=(t'_MS/t_MS)*t`), that the convective-core case (`M>1.3`) gives genuinely *more* rejuvenation than the radiative formula alone would (the "mixes in unburnt fuel" effect, via the surviving `M/M'` factor), that no mass change is a no-op reducing to ordinary fractional aging, clamping at the positive floor, and the expected direction (more mass gained -> more rejuvenation). Wiring tests confirm `evolve()` actually uses this (not the old full-reset) when the companion is genuinely MS-phase at `rlof_time`, and correctly *falls back* to the full-reset simplification when it is not (`classify_rlof()` places no phase constraint on the companion for `STABLE_MASS_TRANSFER`, so a non-MS companion, e.g. already past its own `t_BGB`, is a real case to guard, not a hypothetical one). I verified sensitivity directly, twice: disabled the MS-phase guard (both wiring tests fail as expected, reverted), and forced the mass-ratio factor to always be 1 in `rejuvenate_ms_gainer` (both direction-check tests fail as expected, reverted). **A real, hand-constructed-test-only limitation, found and documented rather than hidden**: exhaustively searched (dozens of donor/companion/separation combinations) for a *naturally find_rlof_onset-discovered* `STABLE_MASS_TRANSFER` crossing where the companion is still MS-phase at a non-trivial (non-zero) `rlof_time`, and found none -- the same structural reason `STABLE_MASS_TRANSFER` itself rarely reaches `evolve()` at all (Section 6's "Residual, understood degeneracy" note): a lighter donor's own much longer MS+HG evolution means that by the time it naturally reaches RLOF, the heavier companion has almost always already evolved off its own (shorter) MS. The wiring tests therefore hand-construct the scenario directly (bypassing `find_rlof_onset`'s own donor selection) rather than relying on a realistic population to reach it -- the rejuvenation code path itself is verified correct; how often a real population actually exercises it is a separate, honestly-reported open question. Brček, Hirai, Mandel & Lower (2026)'s own newer refinement was NOT used here -- flagged explicitly to the user as unverifiable from memory (a very recent, specialized paper) before implementing anything, and Tout et al. (1997)'s older, paper-verified formula was used instead by explicit user decision. |
| `COMMON_ENVELOPE` outcome in `evolve()`'s Phase 0 | HTP02 eqs. 69-73 (energy-balance solve, see Section 12a below) | `binaries/population.py::evolve`, `binaries/interaction.py::apply_common_envelope` | **unit-tested, sensitivity-verified, wired into both consequence branches**: `tests/test_rlof_wiring.py` confirms `evolve()` calls `apply_common_envelope` and applies the result -- survival strips the donor to its core mass and resets *only its own* lifetime clock, tightens the orbit to `a_f`; merger routes the donor's *core* mass (not its pre-CE full mass) plus the companion through the same `did_merge`/`merge_time`/lifetime-reset pathway as `IMMEDIATE_MERGER`. A separate test confirms the `config.alpha_ce`/`lambda_ce` override actually reaches `apply_common_envelope` inside `evolve()` (a low `alpha_ce` forces a merger that would otherwise survive at the default), not just that the config field itself is set. **Real bug found (while implementing B3) and fixed (2026-08-25, by explicit user decision)**: CE-survival used to reset *both* lifetime clocks, including the companion's -- but `apply_common_envelope`'s own docstring says the companion is "unaffected" by a surviving CE (mass unchanged), so resetting its clock incorrectly de-aged it (pretended it just formed anew). Fixed by leaving the companion's clock entirely untouched in the survive branch (not reset, and not rejuvenated -- B3's rejuvenation formula doesn't apply here since the companion has zero mass change, unlike `STABLE_MASS_TRANSFER`'s genuine partial gain). No real stellar-track scenario currently reaches `survives=True` (confirmed by an extensive search across donor/companion/age combinations on the supported HG range -- see Section 12a's merger-dominance finding), so `tests/test_rlof_wiring.py::test_evolve_common_envelope_survives_leaves_companion_clock_untouched` mocks `apply_common_envelope` directly to exercise `evolve()`'s wiring logic in isolation. I verified sensitivity directly: reverted to the old unconditional both-clocks reset, confirmed this test fails exactly as expected (companion clock changes from the sentinel value), reverted back. |
...
## 14. Remaining Paper 1 figures (C2/C3/Figure 5/Figure 6, `docs/science/paper1-detailed-work-breakdown.md`)

| Quantity | Source | Realta implementation | Status |
|---|---|---|---|
| Figure 4 -- `(alpha_IMF, f_bin)` degeneracy grid, `L_X/L_UV` at selected ages (`docs/science/research-programme.md`, "Figure 4 -- IMF versus binary degeneracy") | Not a Power et al. prescription -- a new observable-degeneracy check for this milestone, using A4's continuous Salpeter slope and A1's `binary_fraction` (both already implemented) | `scripts/figure4_imf_binary_grid.py`, `configs/figure4_imf_binary_grid.yml` -- sweeps `imf_slope` (`imf_type` forced to 1/Salpeter, the only continuous-slope preset -- see `imf/factory.py::get_imf`'s docstring) x `binary_fraction`, reading `L_X`/`L_UV` directly from `ClusterSimulation.run()`'s own per-timestep output (A2/A3) at the nearest available timestep to each requested age (not interpolated) | **done, smoke-tested**: `tests/test_figure4_imf_binary_grid.py` covers config parsing/validation (including rejecting `imf_slope`/`binary_fraction`/`imf_type` set directly in the `base` block, since those are supplied per grid point) and a real, small-scale grid run confirming `f_bin=0.0` (no binaries at all) gives `L_X/L_UV=0` at every `alpha_IMF` (no HMXB channel possible without a companion) -- a real physics sanity check, not just "doesn't crash". No interaction prescription is varied (`use_rlof_classifier`/`use_post_sn_rlof` both left at their default-off state) -- Figure 4 is specifically about the IMF/binary-fraction degeneracy, not interaction physics. **Known limitation, not smoothed over**: the default grid config uses `ntot=30000` per point for speed (a 5x5 grid, ~13s total) -- steep-IMF cells (e.g. `alpha_IMF=3.05`) produce very few massive stars (as few as 6 in one run), giving visibly noisy, small-number-statistics cells in the resulting figure rather than smooth degeneracy contours; a production-quality version would need a larger `ntot`, not attempted here since this milestone only required a working, physically-sensible grid, not a publication-precision one. |
| Figure 3 -- effect of mergers: no mergers / standard mergers / enhanced mergers, luminosity evolution AND compact-object formation (`docs/science/research-programme.md`, "Figure 3 -- Effect of mergers") | Not a Power et al. prescription. Prescription mapping: `non_interacting` (no merger channel at all) / `standard_interaction` / `enhanced_mergers` (`config.py::_PRESCRIPTION_DEFAULTS`) | `scripts/figure3_merger_effects.py`, `configs/figure3_merger_effects.yml` -- luminosity panel reads `lumx_tot` directly from `results` (A2/A3); compact-object panel plots cumulative `ndead` (both stars compact) alongside a cumulative merger count built from `BinaryPopulation.merge_time` (set for every merger, whether from the formation-time `p_merge` channel or an RLOF-classifier-driven `IMMEDIATE_MERGER`/CE-merge event) | **done, smoke-tested**: `tests/test_figure3_merger_effects.py` covers config parsing, that `non_interacting` gives exactly zero cumulative mergers throughout (no merger channel active at all), that `enhanced_mergers` produces real, nonzero, monotonically-non-decreasing cumulative merger/`ndead` counts, and figure generation. **Named scope limitation**: "compact-object formation" is reported as cumulative merger count + cumulative `ndead`, not a WD/NS/BH-type census -- Realta has no explicit compact-object-type classifier (`RemnantTable.get_remnant_mass` returns a mass only, no type label), and inventing a mass-threshold-based WD/NS/BH split for this figure alone, without a citable source, was avoided rather than guessed. **Confirmed, not hidden, consequence of the already-documented Xu et al. cross-check finding (Section 10)**: `standard_interaction` and `enhanced_mergers` show near-identical merger/`L_X` curves in the rendered figure (visually overlapping) -- the same automatic-donor-selection property (`q1>1` almost always, regardless of `q_crit_ms`) that made the Xu et al. cross-check merger-heavy also makes `q_crit_ms=0.695` vs `0.4` barely distinguishable here; not tuned away, reported as-is. |
| Figure 5 -- metallicity sweep: repeat the principal experiment for several `Z`, show whether the binary signature survives (`docs/science/research-programme.md`, "Figure 5 -- Metallicity") | Not a Power et al. prescription -- a new cross-check for this milestone | `scripts/figure5_metallicity_sweep.py`, `configs/figure5_metallicity_sweep.yml` -- sweeps `imetal` (1, 2, 3 -- Realta's full `Z=0, 0.008, 0.02` range) x two prescriptions (`non_interacting` vs. `standard_interaction`, isolating the binary-interaction signature itself, not the merger-specific comparison Figure 3 already covers), one panel per `Z`, both curves overlaid, reading `L_X`/`L_UV` directly from `ClusterSimulation.run()`'s own per-timestep output (A2/A3), same convention as Figure 2 | **done, smoke-tested, and visually inspected end-to-end at the config's real `ntot=100000`**: `tests/test_figure5_metallicity_sweep.py` covers config parsing/validation (rejecting `imetal`/`binary_prescription` set directly in `base`, supplied per run instead) and a real, small-scale two-metallicity sweep. Running the actual config end-to-end (`/tmp/figure5_run`, not committed) confirms the physically-sensible, already-documented pattern from Section 10's Xu et al. cross-check and Section 14's Figure 3 entry: at `imetal=2`/`3`, `standard_interaction` tracks `non_interacting` closely and sits SLIGHTLY BELOW it rather than above (the RLOF classifier's donor-selection property occasionally converts an activatable binary into a merger, net removing it from the `fsur` HMXB channel, rather than boosting it) -- not a new finding, the same one from Sections 10/14 now confirmed to persist across metallicity. **At `imetal=1` (Z=0), the two curves are visually identical**, exactly as `config.py`/`binaries/population.py`'s own pre-existing, documented `use_rlof_classifier` handling predicts (RLOF classification is skipped entirely at Z=0, logging a warning, so `standard_interaction` degenerates to `non_interacting`'s own behaviour there) -- this is flagged explicitly in the config file's own header comment as "the binary signature itself is not modelled at Z=0, not that it vanishes physically", not presented as a real physics result. |
| **Figures 1/2 updated (2026-08-25) to show every implemented HMXB-activation mode**, not just the original five `binary_prescription` values (`docs/science/research-programme.md`, "Paper 1 basic experiment") | Not a Power et al. prescription -- `post_sn_rlof`/`wind_capture` are the two later-added, independent opt-in channels (`config.use_post_sn_rlof`/`config.use_wind_capture`), explicit user request | `scripts/run_paper1_experiment.py::VARIANT_OVERRIDES` -- `"post_sn_rlof"`/`"wind_capture"` are NOT `binary_prescription` values, so `run_variant` resolves them separately: each layered on top of the `non_interacting` base (per the user's own explicit choice of 7 standalone curves over a 5x2x2 combinatorial cross with the existing prescriptions), added to `configs/paper1_basic_experiment.yml`'s own sweep list | **done, smoke-tested, and visually inspected end-to-end at the config's real `ntot=100000`**: `tests/test_paper1_pipeline_regression.py` gains 3 new tests (both new variants produce non-degenerate `L_X`, `VARIANT_OVERRIDES`'s keys are disjoint from `config.py::BINARY_PRESCRIPTIONS` -- confirming they can never collide with a real prescription name, and `use_post_sn_rlof`/`use_wind_capture` set directly in `base` is rejected the same way `binary_prescription` already was). Running the actual config end-to-end (`/tmp/paper1_run`, not committed) confirms `post_sn_rlof` gives visibly the STRONGEST `L_X`/`L_X/L_UV` signature of all seven curves (consistent with it being documented, Section 2, as "the physically dominant real HMXB-formation channel"), with `wind_capture` a clearly visible second, both well above the `fsur`-only baseline curves (`non_interacting`/`standard_interaction`/`enhanced_interaction`, which overlap closely, as already documented in Section 10/14's donor-selection finding) -- a physically sensible ordering, not just "doesn't crash". |
| Figure 6 -- stochastic realisations, `P(L_X/L_UV \| M_cl, Z, t)` for finite cluster masses (`docs/science/research-programme.md`, "Figure 6 -- Stochastic realisations") | Not a Power et al. prescription -- explicitly named in the research programme as "the bridge to Paper 2", out of scope for Paper 1's own milestone; built anyway as a deliberately lightweight first look per an explicit user request (2026-08-25) | `scripts/figure6_stochastic_realisations.py`, `configs/figure6_stochastic_realisations.yml` -- NOT Paper 2's parameter-sweep/experiment-runner machinery (the `Event`/`PopulationHistory` abstraction, `docs/science/development-roadmap.md` item 4, still does not exist) -- just re-runs the SAME config `n_realizations` times per cluster-mass proxy (`ntot_values`; the actual M_cl label on each figure panel is read from each realization's own `BinaryPopulation.total_mass_msun`, not the `ntot` input itself), varying only `iseed`, reading `L_X`/`L_UV` at `selected_ages` off each run, and plotting the resulting `log10(L_X/L_UV)` spread as a violin per cluster-mass group. A realization with zero active HMXBs (`L_X=0`, a real outcome for a small enough cluster, not an error) cannot contribute a finite log-ratio point -- reported as an explicit "quiescent fraction" annotation per group instead of being silently dropped from the violin | **done, smoke-tested, and visually inspected end-to-end at the config's real values (3 cluster-mass groups x 40 realizations = 120 runs, ~2m40s)**: `tests/test_figure6_stochastic_realisations.py` covers config parsing/validation (rejecting `ntot`/`iseed` set directly in `base`), a real small-scale two-mass-group run confirming the realized mean cluster mass scales with `ntot` as expected, and a direct physical sanity check that a much smaller cluster has a strictly higher (never lower) quiescent-HMXB fraction at fixed age than a much larger one, using `fsur=1.0` to isolate pure finite-N sampling noise from `fsur` itself. Running the real config end-to-end confirms the expected qualitative result: the smallest group (`M_cl~1719 Msun`) shows a MUCH wider `log10(L_X/L_UV)` spread (full range ~3.3 dex at t=30 Myr) than the largest (`M_cl~27728 Msun`, ~0.4 dex) -- visibly narrowing with increasing cluster mass, the actual physical point of this figure -- though the quiescent fraction happened to be 0% at every group/age tested here (this run's `fsur=0.5`/`mcut=8` combination keeps at least one HMXB active in every realization, even at the smallest mass; the dedicated unit test above uses `fsur=1.0` at a much smaller `ntot=200` specifically to also exercise the nonzero-quiescent-fraction case directly, since the real config's own numbers don't happen to reach it). |

## 15. Wind-capture accretion

Requested by the user with two source papers attached (2026-08-25),
during Figure 3's implementation: a physically-motivated wind-capture
model, "based primarily on El Mellah & Casse (2017), with the wind-
driving/orbital physics informed by Friend & Castor (1982)". Both
papers pasted and read directly, section by section, before
implementing (not from memory) -- per this project's established
discipline for dense/precise physics, reinforced hard by three real
errors this specific implementation caught in itself (below).

| Quantity | Source | Realta implementation | Status |
|---|---|---|---|
| CAK wind mass-loss rate `Mdot_wind`, terminal velocity, velocity profile `v(r)` | El Mellah & Casse (2017, MNRAS, arXiv:1609.01532) eq. (20) (Gayley 1995's Q-parametrization) and eq. (7) (their own calibrated fit, valid for T>21,000K -- every realistic OB-supergiant HMXB donor) | `stellar/cak_wind.py::wind_mass_loss_rate`/`wind_terminal_velocity`/`wind_velocity` | **unit-tested, sensitivity-verified, calibrated against Friend & Castor's own Vela X-1 data (their Table 1: M=24, R=35 Rsun, L=5e5 Lsun, Gamma_*=0.50, observed v_inf=700-1700 km/s, Mdot=0.6-2e-6 Msun/yr)**: `tests/test_cak_wind.py` confirms `eddington_factor` recovers Gamma_*=0.50 to ~8%, `wind_terminal_velocity` (eq. 7) lands inside the observed 700-1700 km/s range, and `wind_mass_loss_rate` (Q~900, El Mellah & Casse's own OB-supergiant fiducial) is within ~2 orders of magnitude of the observed rate (not exact -- both papers themselves flag `Q`/`alpha` as needing per-star calibration, not a generic guess). I verified sensitivity directly: reverted eq. (7) back to the more basic eq. (5) point-source form, confirmed the Vela X-1 calibration test fails exactly as predicted (eq. 5 undershoots the observed range for these parameters -- the actual reason eq. 7 was chosen, not an arbitrary preference), reverted back. **Three real errors caught and fixed while implementing, not left in**: (1) a first-draft `eddington_luminosity` used a memorized "L_Edd(1 Msun)=3.2e4 Lsun" shortcut that turned out to implicitly assume `kappa_e=0.4` (pure hydrogen), inconsistent with this module's own `kappa_e=0.34` (X=0.7) default -- caught by direct numerical recomputation from cgs constants (giving 3.84e4 Lsun instead), not trusted from memory. (2) a first-draft `wind_mass_loss_rate` used a memorized "`L_Edd/c^2 = 1.5e-8 Msun/yr` per Lsun" shortcut that was wrong by ~4 orders of magnitude (that figure is closer to a *typical-efficiency Eddington accretion rate*, a different quantity) -- caught the same way, replaced with a fully explicit cgs-constant derivation (see `cak_wind.py`'s own module docstring). (3) the eq. 5 vs. eq. 7 velocity-law mismatch above. |
| Wind-capture accretion rate onto the compact object | El Mellah & Casse eq. (18) (their own recommended simplified estimate -- beta=0.77*(R_acc/a)^2, evaluated using the wind velocity AT THE ORBITAL SEPARATION, stated to recover their full numerical result to within 6%) -- NOT their eq. (19), whose own text says it underestimates by "at least a factor of three" | `binaries/wind_capture.py::bhl_accretion_fraction`/`wind_capture_rate`, plus `bhl_accretion_rate_simple` (the "simpler BHL-style fallback for comparison" this was scoped to include, using a caller-supplied wind velocity, e.g. terminal velocity, instead of the orbital-separation refinement) | **unit-tested, sensitivity-verified**: `tests/test_wind_capture.py` confirms the expected directions (faster relative wind -> smaller accretion radius; more massive compact object -> larger accretion radius; wider orbit -> smaller captured fraction), clamping at `beta<=1` for an unphysically close/slow configuration, and that the "simple fallback" function is exactly the same composition of the other functions (so the two estimators are directly comparable, isolating "wind speed at separation vs. terminal" as the only difference between them). |
| Circularization radius | NOT from either source paper -- El Mellah & Casse compute this numerically only (their Fig. 6), giving no closed-form fit to transcribe. Uses the standard wind-accretion literature scaling instead (Shapiro & Lightman 1976-type argument: `R_circ/R_acc ~= (1/4)*(v_orbital/v_rel)^4`) | `binaries/wind_capture.py::circularization_radius_fraction`/`circularization_radius` | **unit-tested, sensitivity-verified, but explicitly the LOWEST-confidence piece of this addition**: `tests/test_wind_capture.py` covers the expected direction (faster orbital shear -> larger `R_circ/R_acc`), a zero-orbital-velocity edge case, and a precise pinned value (needed because a looser order-of-magnitude-only test failed to catch a wrong exponent -- see below). **A real, two-stage error found and only partially resolved**: a first-draft formula (specific angular momentum `l=Omega*R_acc^2/4` from a "shear across the cylinder width" argument, then `R_circ=l^2/(G*M)`) gave `R_circ/R_acc~2e-7` for representative Vela-X-1-like parameters -- four orders of magnitude below El Mellah & Casse's own stated range (their Sec. 4.4: `R_circ/R_acc~1e-3` to `1e-2`) -- caught by direct numeric evaluation, not left in. Replaced with the `(v_orbital/v_rel)^4` scaling above, which lands much closer (~1.4e-4 for the same numbers) but is STILL below their stated range and its exact prefactor (taken as 1/4, the commonly-cited value) has not been independently verified against a pasted source, unlike everything else in this addition -- flagged explicitly in the function's own docstring rather than presented as settled. I also verified sensitivity directly: the initial order-of-magnitude-only test (`1e-5 < frac < 1e-1`) did NOT catch a deliberately-broken exponent (`^2` instead of `^4`) -- added a second, precisely-pinned-value test that does, confirmed it catches the same break, reverted. |
| **Wiring into `BinaryPopulation.evolve()`** (2026-08-25 follow-up) | n/a -- design decision made in-repo, not paper-sourced | New Phase 1.5 branch, `config.use_wind_capture` (opt-in, default `False`): gated on `nturn==1` (primary already compact) and `donor_radius < r_l2` (secondary donor NOT yet filling its Roche lobe) -- the pre-RLOF regime `use_post_sn_rlof`'s own trigger (`donor_radius >= r_l2`) does not cover. Both flags share one loop over the same donor-phase/radius/Roche-lobe setup; either can run alone, or together for one continuous `nturn==1` pipeline (wind-fed while detached, handing off to `use_post_sn_rlof`'s certain activation once the donor actually overflows). `L_X = eta*Mdot_acc*c^2` (`eta=0.1`, reusing `xray/luminosity.py::XRayLuminosity.eta` -- a pre-existing but previously-unused attribute of the same class that already provides this branch's Eddington cap, `eddington_luminosity(m1)`, rather than introducing a second, redundant efficiency parameter), Eddington-capped, stored `lunit`-normalized like every other `lum_xray` value in this module. Two new config fields, `wind_cak_alpha=0.55` (midpoint of El Mellah & Casse's cited 0.45-0.65 OB-supergiant range) and `wind_cak_q_force=900.0` (their own stated typical value) -- both validated in `__post_init__`. **Design note on "smoothly approach RLOF as the donor fills its critical lobe" (the user's own original requirement)**: not a separate, hand-tuned enhancement term -- `cak_wind.wind_velocity(a, donor_radius, v_inf)` (the CAK wind speed AT THE ORBITAL SEPARATION, El Mellah & Casse's own eq. 18 convention) falls as `donor_radius` grows toward the separation, which raises `bhl_accretion_fraction` (`~1/v_rel^2`) continuously right up to `use_post_sn_rlof`'s own discrete hand-off -- an emergent consequence of the existing formulae, not new physics. Activated once and held fixed for the rest of the binary's active lifetime (matching this module's existing convention for `fsur`/`use_post_sn_rlof`), not recomputed every timestep as the donor evolves -- a deliberate simplification, consistent with the rest of this module, not an oversight. |
| **Wiring test suite** | n/a | `tests/test_wind_capture_wiring.py` | 10 tests: config default off, inert when disabled, produces a positive sub-Eddington luminosity for a Vela-X-1-like hand-constructed scenario, closer orbit -> higher captured luminosity (direction check), `use_post_sn_rlof` takes over (confirmed via `unittest.mock.patch.object` wrapping `xray_calc.get_lumx`, not a magnitude bound -- the RLOF draw is not confined to `[lxmin, lxmax]`, see `xray/luminosity.py`) once the donor fills its Roche lobe, wind-capture does NOT activate at all in that same tight-orbit case with `use_post_sn_rlof` off (hard gate, not preference), `imetal=1` graceful skip with a logged warning, "activate once, hold fixed" (a second `evolve()` call does not overwrite an already-set value), `mcomp` floor, and a direct sensitivity check that `wind_cak_alpha` is actually wired through (not silently ignored). I verified sensitivity directly on the gate itself: loosened `donor_radius < r_l2` to `donor_radius < r_l2*1e9` (effectively removing it), confirmed `test_wind_capture_applies_only_while_donor_is_below_its_roche_lobe` fails exactly as expected, reverted. |
...
- ~~Figure 3 (mergers vs. compact-object formation) not built~~ -- now
  built (2026-08-25, `scripts/figure3_merger_effects.py`) -- see
  Section 14.
- ~~Figures 4-6 out of scope for this milestone~~ -- Figures 4 and 5
  (IMF/binary degeneracy grid, metallicity sweep) are now both built
  (2026-08-25, Section 14). Figure 6 (stochastic realisations) got a
  lightweight first look too (2026-08-25, Section 14, explicit user
  request), but Paper 2's actual parameter-sweep/experiment-runner
  machinery (Phase 3, `docs/science/development-roadmap.md` item 4)
  that a production version needs is still NOT implemented -- the full
  `Event`/`PopulationHistory` abstraction remains genuinely open, only
  the minimal `did_merge`/`merge_time` event bookkeeping Figure 3
  needed exists (Section 6). `scripts/figure6_stochastic_realisations.py`
  is a standalone repeat-seed runner, not that machinery -- see its
  own module docstring and Section 14 for the distinction.
...
- ~~Post-SN secondary Roche-lobe overflow not modelled (found
  ...
  above)~~ -- the RLOF-only version is now implemented
  (`config.use_post_sn_rlof`, Section 2) -- see below. Originally: Phase
  0's RLOF classifier only checks pre-SN interaction between two
  still-evolving stars (`nturn==0`), matching HTP02's own CE-eligible
  donor list. Because `generate_population` always enforces `m2 <= m1`,
  and `STABLE_MASS_TRANSFER` requires the donor to be the *lighter*
  star, that channel's donor is always `m2` -- whose own pre-SN
  evolutionary timescale is always far longer than `m1`'s (the only
  star whose SN Realta's `turnoff_time` tracks), so `m1` has
  essentially always already exploded before this pre-SN channel
  ...
  The astrophysically dominant real HMXB-formation channel is the
  ...
  not pre-SN mass transfer between two still-live stars. Implemented
  (2026-08-25) as a new "Phase 1.5" in `evolve()`, gated on `nturn==1`,
  donor = the still-evolving secondary, using the same Eggleton Roche-
  lobe machinery as Phase 0 (a compact object has no stellar radius to
  overflow *from*, so only the secondary's own radius-vs-Roche-lobe
  check is needed -- no new physics required for the trigger itself).
  Wind accretion (the *other* real HMXB-formation channel, not
  requiring Roche-lobe overflow at all) is a separate addition -- see
  Section 15: the physics modules (`stellar/cak_wind.py`,
  `binaries/wind_capture.py`) exist, are independently tested, AND (as
  of 2026-08-25) are wired into `evolve()`'s HMXB-activation pipeline
  (`config.use_wind_capture`), composing with `use_post_sn_rlof` into
  one continuous `nturn==1` pipeline -- see Section 15's "Wiring into
  `BinaryPopulation.evolve()`" row for the full writeup. Nothing about
