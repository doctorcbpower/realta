...
## 15. Wind-capture accretion (physics modules, not yet wired into `evolve()`)
...
| **Wiring into `BinaryPopulation.evolve()`** | n/a | Not done | **Deliberately NOT wired in yet** -- built as standalone, independently-tested physics modules only, per the user's own "keep the prescription modular" framing. Wiring this into the HMXB-activation pipeline needs its own explicit design decision (does it replace, gate, or run alongside the existing `fsur`/`use_post_sn_rlof` channels? how does `L_X` get computed from `Mdot_acc` -- a new `eta*Mdot_acc*c^2`-style conversion, per Friend & Castor eq. 12/13, or fed through the existing `xray_calc` luminosity-draw machinery?) -- not resolved here, flagged as the explicit next step. |
